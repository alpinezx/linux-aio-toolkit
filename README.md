...
- One script, one menu, that keeps returning to itself after each action, so you can run several things back to back without relaunching. A couple of submenus (Throughput tuning) go a step further and loop on their own too, so picking several related options in a row doesn't mean re-navigating the main menu each time.
- Every module is **idempotent**: safe to re-run any time, and checks what's already done before doing anything.
- Where it makes sense, changes are **reversible**: BBR, the `tuned` profile, and TCP buffer tuning can all be put back to what they were before, not just applied.
...
```
 ...
 3) Harden server (fail2ban / UFW / automatic security updates, each optional)
 4) TCP congestion control (enable BBR / revert to CUBIC)
 5) Throughput tuning (tuned profile + TCP write buffers, revert available)
 6) DNS-over-TLS (Cloudflare / Quad9 / custom, bypasses provider DNS)
 7) Set up scheduled reboot (daily/weekly/monthly) + clock sync (NTP)
 8) Check current scheduled reboot status (read-only, no prompts)
 9) Check root SSH login status (across every sshd config file, not just one)
10) Create a non-root sudo user (adds them, sudo group, copies root's SSH key)
11) Enable root SSH login (copies your key, sets PermitRootLogin, restarts sshd)
12) Passwordless sudo for a user (real security trade-off, reads warnings)
13) Disable root SSH login (locks it down, reads warnings before proceeding)
14) Remove a user (permanent, reads warnings, requires typed confirmation)
15) Exit
```
...
- **Harden server.** fail2ban (with the `ssh.service` vs `sshd.service` unit-detection fix baked in), UFW with base ports (22/80/443 by default, override via `UFW_PORTS`), and unattended-upgrades (security patches only, no auto-reboot). Each piece is asked about independently, so any combination is fine.
- **TCP congestion control.** A small submenu: switch from CUBIC to BBR (plus the `fq` qdisc), testable live before persisting across reboots, or revert cleanly back to CUBIC. Reports current state either way and is a no-op if there's nothing to change.
- **Throughput tuning.** A second, looping submenu (stays open across multiple selections, pausing after each, until you choose "back to main menu"):
  - *Apply the `tuned` `throughput-performance` profile* — installs `tuned` if missing, switches the profile, and verifies. On VPS shapes where the hypervisor doesn't expose CPU frequency scaling to the guest (common on KVM-based cloud shapes), the governor-related part of the profile silently no-ops; the I/O read-ahead, write-batching, and memory-tuning parts still apply. On bare metal or where cpufreq is exposed, the whole profile takes effect.
  - *Balance TCP write buffers (wmem) up to match read buffers (rmem)* — Ubuntu's stock kernel defaults leave the write ceiling well below the read ceiling, which matters specifically for anything proxying data **out** to clients (video streaming, file serving), since the write side governs relay throughput. Raises it to 16MB, live and persisted.
  - *Revert the `tuned` profile* — switches back to whatever profile was active before this toolkit first touched it (remembered in a small state file, not guessed), falling back to `tuned`'s generic `balanced` profile if no record exists.
  - *Revert TCP write buffers* — restores Ubuntu's stock `wmem_max`/`tcp_wmem` values, live and persisted.
...
- **Scheduled reboot.** Optional timezone change, a reboot cron job (daily, weekly, or monthly, at a time you choose), and NTP sync via systemd-timesyncd so the schedule doesn't drift.
- **Check scheduled reboot status.** Read-only. Shows whatever reboot schedule the toolkit currently owns, translated into plain English, without touching timezone, frequency, or NTP.
...
- **Throughput changes know what they're reverting to, not guessing.** The `tuned` profile revert reads back the actual profile that was active before this toolkit first switched it, saved to a small state file the first time it runs, rather than assuming a generic default. If that record is ever missing, it says so explicitly before falling back.
...
- On virtualized shapes where the hypervisor doesn't expose CPU frequency scaling to the guest (Oracle's KVM-based A1 Flex, for example), `tuned-adm verify` may report warnings about missing cpufreq/sysfs paths after applying the throughput-performance profile. That's expected on those shapes, not a failure, everything else in the profile still applies. See the Throughput tuning entry above.
