...
- A fresh Ubuntu or Debian VPS (tested on Ubuntu 24.04 LTS; should work on other Debian-based distros, but paths/behavior may differ slightly)
...
```
 ...
 5) Set up scheduled daily reboot + clock sync (NTP)
 6) Check root SSH login status (across every sshd config file, not just one)
 7) Create a non-root sudo user (adds them, sudo group, copies root's SSH key)
 8) Enable root SSH login (copies your key, sets PermitRootLogin, restarts sshd)
 9) Passwordless sudo for a user (real security trade-off — reads warnings)
10) Disable root SSH login (locks it down — reads warnings before proceeding)
11) Remove a user (permanent — reads warnings, requires typed confirmation)
12) Exit
```
