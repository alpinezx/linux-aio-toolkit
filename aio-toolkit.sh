...
# Where modules that offer a revert (tuned profile, TCP buffers, ...) stash
# small "what was it before we touched it" marker files. Keeps revert logic
# honest about the actual prior state instead of guessing a generic default.
AIO_STATE_DIR="/etc/aio-toolkit"
mkdir -p "$AIO_STATE_DIR" 2>/dev/null || true
...
# ============================================================
# ---------- module: throughput tuning ----------
# Two independent, idempotent pieces pulled from a manual tuning pass done
# on a media-proxy box: the `tuned` throughput-performance profile (I/O
# read-ahead, dirty-page/write-batching, memory reclaim behaviour — CPU
# governor bits inside it silently no-op on hypervisors that don't expose
# cpufreq to the guest, e.g. Oracle's KVM-based shapes, which is fine and
# expected, not a failure) and balancing the TCP write-buffer ceiling
# (wmem) up to match the read-buffer ceiling (rmem) — Ubuntu's default
# leaves wmem far lower, which matters specifically for anything proxying
# data OUT to clients (video streaming, file serving), since the write
# side is what actually governs upload/relay throughput.
#
# Both pieces have a matching revert. The tuned revert reads back whatever
# profile was active before this toolkit first switched it (saved to
# AIO_STATE_DIR the first time setup_tuned_profile() runs) rather than
# guessing a default. The buffer revert restores Ubuntu's stock tcp_wmem/
# wmem_max values, live and persisted.
# ============================================================

# Installs `tuned` if missing and sets the throughput-performance profile.
# Idempotent: reports current profile and does nothing further if it's
# already active.
setup_tuned_profile() {
    local target_profile="throughput-performance"
    local current_profile

    info "tuned throughput-performance profile"

    if ! dpkg -s tuned >/dev/null 2>&1; then
        echo "Installing tuned..."
        apt-get update -qq || { warn "apt update failed — check your network and try again."; return 1; }
        apt-get install -y -qq tuned || { warn "tuned install failed."; return 1; }
    fi

    systemctl enable --now tuned >/dev/null 2>&1 || true

    current_profile=$(tuned-adm active 2>/dev/null | sed -n 's/^Current active profile: //p')
    echo "Current profile: ${current_profile:-unknown}"

    if [[ "$current_profile" == "$target_profile" ]]; then
        echo "Already on ${target_profile} — nothing to do."
        tuned-adm verify || warn "tuned-adm verify reported a mismatch — see /var/log/tuned/tuned.log."
        return 0
    fi

    read -rp "Switch tuned profile to ${target_profile}? [Y/n]: " CONFIRM_TUNED
    CONFIRM_TUNED="${CONFIRM_TUNED:-Y}"
    if [[ ! "$CONFIRM_TUNED" =~ ^[Yy]$ ]]; then
        warn "Aborted. No changes made."
        return 0
    fi

    # Only overwrite the saved "previous profile" the FIRST time — if it's
    # already recorded, this isn't the original state anymore (could be a
    # re-run after already switching once), so don't clobber it.
    if [[ -n "$current_profile" && "$current_profile" != "unknown" && ! -f "${AIO_STATE_DIR}/tuned-previous-profile" ]]; then
        echo "$current_profile" > "${AIO_STATE_DIR}/tuned-previous-profile"
    fi

    tuned-adm profile "$target_profile" || { warn "Failed to set tuned profile."; return 1; }

    echo ""
    echo "Verifying (warnings about missing cpufreq/sysfs paths are expected and harmless"
    echo "on VPS shapes where the hypervisor doesn't expose CPU frequency scaling to the"
    echo "guest — the I/O and memory-tuning parts of the profile still apply)."
    tuned-adm verify || warn "tuned-adm verify reported a mismatch — see /var/log/tuned/tuned.log."
    return 0
}

# Balances the TCP write-buffer ceiling (wmem) up to match a sane read-
# buffer (rmem) ceiling. Idempotent: checks the live value first, and
# separately checks whether it's already persisted in /etc/sysctl.conf —
# mirrors enable_bbr()'s live-vs-persisted distinction above.
tune_tcp_buffers() {
    local target_wmem_max=16777216
    local target_tcp_wmem="4096 65536 16777216"
    local current_wmem_max current_rmem_max current_tcp_wmem persisted

    info "TCP write buffer tuning"
    ...
    current_tcp_wmem=$(sysctl -n net.ipv4.tcp_wmem 2>/dev/null || echo "unknown")
    echo "Current net.core.rmem_max: $current_rmem_max"
    echo "Current net.core.wmem_max: $current_wmem_max"
    echo "Current net.ipv4.tcp_wmem: $current_tcp_wmem"

    persisted=false
    if grep -qE "^net\.core\.wmem_max[[:space:]]*=[[:space:]]*${target_wmem_max}\$" /etc/sysctl.conf 2>/dev/null; then
        persisted=true
    fi

    if [[ "$current_wmem_max" -ge "$target_wmem_max" && "$persisted" == true ]]; then
        echo "Write buffer ceiling is already at or above ${target_wmem_max} bytes and persisted — nothing to do."
        return 0
    fi

    if [[ "$current_wmem_max" -ge "$target_wmem_max" && "$persisted" == false ]]; then
        warn "Write buffers are already raised live but not persisted — it will revert to a lower value on reboot."
        echo "Persisting current value now..."
    else
        warn "Write buffers (wmem) are capped well below read buffers (rmem) by Ubuntu's default —"
        echo "this matters for anything proxying data OUT to clients (e.g. video streaming), since"
        echo "the write side is what governs relay/upload throughput."
        echo ""
        read -rp "Raise wmem ceiling to ${target_wmem_max} bytes (16MB) now, live and persisted? [Y/n]: " CONFIRM_WMEM
        CONFIRM_WMEM="${CONFIRM_WMEM:-Y}"
        if [[ ! "$CONFIRM_WMEM" =~ ^[Yy]$ ]]; then
            warn "Aborted. No changes made."
            return 0
        fi
        sysctl -w net.core.wmem_max="$target_wmem_max" >/dev/null
        sysctl -w net.ipv4.tcp_wmem="$target_tcp_wmem" >/dev/null
        echo "Applied live."
    fi

    if grep -qE '^net\.core\.wmem_max[[:space:]]*=' /etc/sysctl.conf 2>/dev/null; then
        sed -i -E "s/^net\.core\.wmem_max[[:space:]]*=.*/net.core.wmem_max=${target_wmem_max}/" /etc/sysctl.conf
    else
        echo "net.core.wmem_max=${target_wmem_max}" >> /etc/sysctl.conf
    fi

    if grep -qE '^net\.ipv4\.tcp_wmem[[:space:]]*=' /etc/sysctl.conf 2>/dev/null; then
        sed -i -E "s/^net\.ipv4\.tcp_wmem[[:space:]]*=.*/net.ipv4.tcp_wmem=${target_tcp_wmem}/" /etc/sysctl.conf
    else
        echo "net.ipv4.tcp_wmem=${target_tcp_wmem}" >> /etc/sysctl.conf
    fi

    sysctl -p >/dev/null 2>&1 || warn "sysctl -p reported an issue applying /etc/sysctl.conf — check it for unrelated bad lines. Buffer settings were still written to the file."
    echo "Persisted. Confirmed via:"
    sysctl net.core.wmem_max net.ipv4.tcp_wmem
    return 0
}

# Reverts the tuned profile back to whatever was active before
# setup_tuned_profile() first switched it (read from the marker file it
# saves). If no marker exists — this toolkit never made the switch, or the
# marker's gone missing — falls back to tuned's own generic "balanced"
# profile instead of guessing, and says so.
revert_tuned_profile() {
    local current_profile saved_profile

    info "tuned profile: revert"

    if ! dpkg -s tuned >/dev/null 2>&1; then
        echo "tuned isn't installed — nothing to revert."
        return 0
    fi
    ...
    if [[ -f "${AIO_STATE_DIR}/tuned-previous-profile" ]]; then
        saved_profile=$(cat "${AIO_STATE_DIR}/tuned-previous-profile")
    fi

    if [[ -z "${saved_profile:-}" ]]; then
        warn "No record of a prior profile (this toolkit may never have changed it here)."
        saved_profile="balanced"
        echo "Falling back to tuned's generic '${saved_profile}' profile instead."
    fi

    if [[ "$current_profile" == "$saved_profile" ]]; then
        echo "Already on ${saved_profile} — nothing to do."
        return 0
    fi

    read -rp "Switch tuned profile back to ${saved_profile}? [Y/n]: " CONFIRM_REVERT_TUNED
    CONFIRM_REVERT_TUNED="${CONFIRM_REVERT_TUNED:-Y}"
    if [[ ! "$CONFIRM_REVERT_TUNED" =~ ^[Yy]$ ]]; then
        warn "Aborted. No changes made."
        return 0
    fi

    tuned-adm profile "$saved_profile" || { warn "Failed to set tuned profile."; return 1; }
    rm -f "${AIO_STATE_DIR}/tuned-previous-profile"

    echo "Reverted. Confirmed via:"
    tuned-adm active
    return 0
}

# Reverts the TCP write-buffer settings tune_tcp_buffers() applied: removes
# the sysctl.conf lines it added/edited AND explicitly sets the live values
# back down (mirrors revert_to_cubic()'s note above about fq — here there's
# no equivalent leftover to worry about, since removing the file lines
# alone wouldn't touch the already-live kernel values until next reboot).
# Ubuntu's own stock default for tcp_wmem is "4096 16384 4194304" — that's
# what's restored live; no separate default exists for wmem_max itself
# beyond what tcp_wmem's third value implies, so the same number covers
# both.
revert_tcp_buffers() {
    local stock_wmem_max=4194304
    local stock_tcp_wmem="4096 16384 4194304"
    local current_wmem_max persisted

    info "TCP write buffer tuning: revert"
    current_wmem_max=$(sysctl -n net.core.wmem_max 2>/dev/null || echo 0)
    echo "Current net.core.wmem_max: $current_wmem_max"

    persisted=false
    if grep -qE '^net\.core\.wmem_max[[:space:]]*=' /etc/sysctl.conf 2>/dev/null \
        || grep -qE '^net\.ipv4\.tcp_wmem[[:space:]]*=' /etc/sysctl.conf 2>/dev/null; then
        persisted=true
    fi

    if [[ "$current_wmem_max" -le "$stock_wmem_max" && "$persisted" == false ]]; then
        echo "Already at/below the stock default and nothing persisted — nothing to do."
        return 0
    fi

    read -rp "Revert TCP write buffers to Ubuntu's stock default now (live and persisted)? [Y/n]: " CONFIRM_REVERT_WMEM
    CONFIRM_REVERT_WMEM="${CONFIRM_REVERT_WMEM:-Y}"
    if [[ ! "$CONFIRM_REVERT_WMEM" =~ ^[Yy]$ ]]; then
        warn "Aborted. No changes made."
        return 0
    fi

    sysctl -w net.core.wmem_max="$stock_wmem_max" >/dev/null
    sysctl -w net.ipv4.tcp_wmem="$stock_tcp_wmem" >/dev/null
    echo "Reverted live."

    sed -i -E '/^net\.core\.wmem_max[[:space:]]*=/d' /etc/sysctl.conf
    sed -i -E '/^net\.ipv4\.tcp_wmem[[:space:]]*=/d' /etc/sysctl.conf

    sysctl -p >/dev/null 2>&1 || warn "sysctl -p reported an issue applying /etc/sysctl.conf — check it for unrelated bad lines."
    echo "Persisted (lines removed from sysctl.conf, kernel defaults apply). Confirmed via:"
    sysctl net.core.wmem_max net.ipv4.tcp_wmem
    return 0
}

# Small submenu wrapper, same idea as congestion_control_menu but looping —
# unlike that one, this menu stays open across multiple actions (pausing
# after each) since apply/revert here are commonly checked one after the
# other, and only returns to the main menu on explicit choice.
throughput_tuning_menu() {
    while true; do
        echo ""
        info "Throughput tuning"
        echo "  1) Apply tuned throughput-performance profile"
        echo "  2) Balance TCP write buffers (wmem) up to match read buffers"
        echo "  3) Revert tuned profile to what it was before"
        echo "  4) Revert TCP write buffers to Ubuntu's stock default"
        echo "  5) Back to main menu"
        read -rp "Select an option [1-5]: " TT_CHOICE

        case "$TT_CHOICE" in
            1) setup_tuned_profile || warn "tuned setup did not fully complete — see messages above."; press_enter ;;
            2) tune_tcp_buffers || warn "Buffer tuning did not fully complete — see messages above."; press_enter ;;
            3) revert_tuned_profile || warn "tuned revert did not fully complete — see messages above."; press_enter ;;
            4) revert_tcp_buffers || warn "Buffer revert did not fully complete — see messages above."; press_enter ;;
            5) return 0 ;;
            *) warn "Not a valid option." ;;
        esac
    done
...
while true; do
    ...
    echo "  5) Throughput tuning (tuned profile + TCP write buffers, revert available)"
    echo "  6) DNS-over-TLS (Cloudflare / Quad9 / custom, bypasses provider DNS)"
    echo "  7) Set up scheduled reboot (daily/weekly/monthly) + clock sync (NTP)"
    echo "  8) Check current scheduled reboot status (read-only, no prompts)"
    echo "  9) Check root SSH login status (across every sshd config file, not just one)"
    echo " 10) Create a non-root sudo user (adds them, sudo group, copies root's SSH key)"
    echo " 11) Enable root SSH login (copies your key, sets PermitRootLogin, restarts sshd)"
    echo " 12) Passwordless sudo for a user (real security trade-off, reads warnings)"
    echo " 13) Disable root SSH login (locks it down, reads warnings before proceeding)"
    echo " 14) Remove a user (permanent, reads warnings, requires typed confirmation)"
    echo " 15) Exit"
    echo ""
    read -rp "Select an option [1-15]: " MENU_CHOICE
